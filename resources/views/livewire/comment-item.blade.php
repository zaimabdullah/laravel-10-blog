<div class="flex justify-between mb-4 gap-3 sm:justify-center">
  ...
  <div class="flex-1">
    ...
    @else
      <div class="text-gray-700">
        {{-- $comment = current comment created, ->comment = the content of current comment --}}
        {{ $comment->comment }}
      </div>
    @endif
    <div>
    ...
    {{-- $comment = current comment, ->comments = childs of the current comment --}}
    @if ($comment->comments->count())
      {{-- <pre>{{ $comment->comments->count() }}</pre> --}}
      <div class="mt-4">
